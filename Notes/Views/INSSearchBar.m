...
#define DEFAULT_PURPLE [UIColor colorWithRed:116/255.0f green:66/255.0f blue:186/255.0f alpha:1.0]
...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:CGRectMake(CGRectGetMinX(frame), CGRectGetMinY(frame), CGRectGetWidth(frame), 34.0)]))
	{
		...
		self.searchField = [[UITextField alloc] initWithFrame:
                            CGRectMake(kINSSearchBarInset, 3.0,
                                       CGRectGetWidth(self.bounds) - (2 * kINSSearchBarInset) - kINSSearchBarImageSize,
                                       CGRectGetHeight(self.bounds) - 6.0)];
        ...
        self.searchField.backgroundColor = DEFAULT_PURPLE;
		self.searchField.alpha = 0.0;
		self.searchField.delegate = self;
		
		[self.searchFrame addSubview:self.searchField];
		
		UIView *searchImageViewOnContainerView = [[UIView alloc] initWithFrame:
                                                  CGRectMake(CGRectGetWidth(self.bounds) - kINSSearchBarInset - kINSSearchBarImageSize,
                                                             (CGRectGetHeight(self.bounds) - kINSSearchBarImageSize) / 2,
		...
		self.searchImageViewOn.image = [UIImage imageNamed:@"searchBar_white"];
		...
		self.searchImageCircle.image = [UIImage imageNamed:@"searchBar_Circle"];
		...
		self.searchImageCrossLeft.image = [UIImage imageNamed:@"searchBarLine1"];
		...
		self.searchImageCrossRight.image = [UIImage imageNamed:@"searchBarLine2"];
		[searchImageViewOnContainerView addSubview:self.searchImageCrossRight];

		self.searchImageViewOff = [[UIImageView alloc] initWithFrame:
                                   CGRectMake(CGRectGetWidth(self.bounds) - kINSSearchBarInset - kINSSearchBarImageSize,
                                              (CGRectGetHeight(self.bounds) - kINSSearchBarImageSize) / 2,
                                              kINSSearchBarImageSize,
                                              kINSSearchBarImageSize)];
		...
		self.searchImageViewOff.image = [UIImage imageNamed:@"searchBar_purple"];
		
		[self.searchFrame addSubview:self.searchImageViewOff];

		UIView *tapableView = [[UIView alloc] initWithFrame:
                               CGRectMake(CGRectGetWidth(self.bounds) - (2 * kINSSearchBarInset) - kINSSearchBarImageSize,
                                          0.0,
                                          (2 * kINSSearchBarInset) + kINSSearchBarImageSize,
	...
	if (self.state == INSSearchBarStateNormal)
	{
		...
		[UIView animateWithDuration:kINSSearchBarAnimationStepDuration animations:^{
			...
			[UIView animateWithDuration:kINSSearchBarAnimationStepDuration * 2 animations:^{
				...
				self.searchFrame.layer.backgroundColor = DEFAULT_PURPLE.CGColor;
